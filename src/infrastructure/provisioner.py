...
class InfrastructureProvisioner:
        ...
        try:
            response = self.ec2.run_instances(
                ...
                TagSpecifications=[{
                    ...
                    'Tags': [{'Key': 'Name', 'Value': instance_config.get('name', 'DevOps-Instance')}]
                }]
            )
            instance_id = response['Instances'][0]['InstanceId']
            self.console.print(f"[green]Successfully created EC2 instance: {instance_id}[/green]")
    ...
    def create_s3_bucket(self, bucket_name: str, region: str = 'us-west-2'):
        ...
        try:
            ...
            self.console.print(f"[green]Successfully created S3 bucket: {bucket_name}[/green]")
        except Exception as e:
        ...
        try:
            ...
            if 's3_buckets' in config:
                for bucket in config['s3_buckets']:
                    self.create_s3_bucket(bucket['name'], bucket.get('region', 'us-west-2'))

            self.console.print("[green]Infrastructure provisioning completed![/green]")
        except Exception as e:
            self.console.print(f"[red]Error provisioning infrastructure: {str(e)}[/red]")
