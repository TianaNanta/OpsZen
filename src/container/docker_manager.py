...
class DockerManager:
    ...
    def create_container(
        ...
        try:
            container = self.client.containers.run(
                image, name=name, ports=ports, environment=environment, detach=True
            )
            self.console.print(
                f"[green]Container created successfully: {container.name}[/green]"
            )
            return container
    ...
    def stop_container(self, container_id: str):
        ...
        try:
            ...
            container.stop()
            self.console.print(
                f"[green]Container {container_id} stopped successfully[/green]"
            )
    ...
    def remove_container(self, container_id: str, force: bool = False):
        ...
        try:
            ...
            self.console.print(
                f"[green]Container {container_id} removed successfully[/green]"
            )
        except docker.errors.NotFound:
            self.console.print(f"[red]Container {container_id} not found[/red]")
        except docker.errors.APIError as e:
